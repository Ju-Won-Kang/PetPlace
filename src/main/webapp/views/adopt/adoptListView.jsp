<%@ page language="java" contentType="text/html;charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page
	import="com.petplace.common.PageInfo, java.util.ArrayList, com.petplace.adopt.model.vo.Adopt"%>
...
<body>
	...
	<div class="content">
		<div class="adopt-type-btn">
			<button id="missing-btn" onclick="adoptMissingBtn()">유기 /
				실종&nbsp;&nbsp;</button>
			<p>|</p>
		...
		<form class="find-pet-option" action="adoptSearch.do">
			<div class="select-pet-place">
				<input type="hidden" name="cpage" value="1"> <input
					type="hidden" name="boardType" value="${boardType}"> <select
					id="select-pet-type" name="category">
			...
			<div id="find-pet-area">
				<input type="text" name="keyword" value="${keyword}"
					placeholder="검색">
				<button type="submit">
					<img src="<%=contextPath%>/images/Search.png">
				</button>
			</div>
		</form>
					...
					<c:choose>
						...
						<c:otherwise>
							<c:forEach var="a" items="${list}">
								<tr class="table-line"
									onclick="detailPage(${a.boardNo}, '${a.boardType}')">
									<td>${a.boardNo}</td>
									<td><img
										src="<%= request.getContextPath() %>/${a.adoptImg}"></td>
									<td class="table-title">
										<p>${a.boardTitle}</p> <span id="reply-content">${a.boardDetail}</span>
									</td>
									<td>${a.place}</td>
									<td>${a.findDate}</td>
									<td>${a.memberId}</td>
			...
			<nav>
				<c:if test="${keyword == null}">
					<ul class="pagination">
						<c:choose>
							<c:when
								test="${pi.startPage != 1 || (pi.startPage / pi.boardLimit)  > 1}">
								<li class="page-item"><c:choose>
										<c:when test="${category ne null}">
											<a
												href="adoptSearch.do?cpage=${pi.startPage - pi.boardLimit}&boardType=${boardType}&category=${category}"
												class="page-link"> <span aria-hidden="true">&laquo;</span>
											</a>
										</c:when>
										<c:otherwise>
											<a
												href="adoptSearch.do?cpage=${pi.startPage - pi.boardLimit}&boardType=${boardType}"
												class="page-link"> <span aria-hidden="true">&laquo;</span>
											</a>
										</c:otherwise>
									</c:choose></li>
						...
						<c:choose>
							<c:when test="${pi.currentPage > 1}">
								<li class="page-item"><c:choose>
										<c:when test="${category ne null}">
											<a
												href="adoptSearch.do?cpage=${pi.currentPage - 1}&category=${category}&boardType=${boardType}"
												class="page-link"> <span aria-hidden="true">&lt;</span>
											</a>
										</c:when>
										<c:otherwise>
											<a href="adoptSearch.do?cpage=${pi.currentPage - 1}&boardType=${boardType}"
												class="page-link"> <span aria-hidden="true">&lt;</span>
											</a>
										</c:otherwise>
									</c:choose></li>
						...
						<c:forEach var="page" begin="${pi.startPage}" end="${pi.endPage}"
							...
							<c:choose>
								<c:when test="${page == pi.currentPage}">
									<li class="page-item active"><c:choose>
											<c:when test="${category ne null}">
												<a class="page-link" href="adoptSearch.do?cpage=${page}&boardType=${boardType}&category=${category}">${page}</a>
											</c:when>
											<c:otherwise>
												<a class="page-link" href="adoptSearch.do?cpage=${page}&boardType=${boardType}">${page}</a>
											</c:otherwise>
										</c:choose></li>
								</c:when>
								<c:otherwise>
						...
						<c:choose>
							<c:when test="${pi.currentPage < pi.maxPage}">
								<li class="page-item"><c:choose>
										<c:when test="${category ne null}">
											<a href="adoptSearch.do?cpage=${pi.currentPage + 1}&boardType=${boardType}&category=${category}" class="page-link"> 
												<span aria-hidden="true">&gt;</span>
											</a>
										</c:when>
										<c:otherwise>
											<a href="adoptSearch.do?cpage=${pi.currentPage + 1}&boardType=${boardType}" class="page-link"> 
												<span aria-hidden="true">&gt;</span>
											</a>
										</c:otherwise>
									</c:choose></li>
						...
						<c:choose>
							<c:when test="${(pi.endPage / boardLimit)  < pi.maxPage}">
								<li class="page-item"><c:choose>
										<c:when test="${category ne null}">
											<a href="adoptSearch.do?cpage=${pi.endPage + 1}&boardType=${boardType}&category=${category}" class="page-link"> 
												<span aria-hidden="true">&raquo;</span>
											</a>
										</c:when>
										<c:otherwise>
											<a href="adoptSearch.do?cpage=${pi.endPage + 1}&boardType=${boardType}" class="page-link"> 
												<span aria-hidden="true">&raquo;</span>
											</a>
										</c:otherwise>
									</c:choose></li>
					...
					<ul class="pagination">
						<c:choose>
							<c:when
								...
								<li class="page-item"><c:choose>
										<c:when test="${category ne null}">
											<a href="adoptSearch.do?cpage=${pi.startPage - pi.boardLimit}&boardType=${boardType}&category=${category}&keyword=${keyword}" class="page-link"> 
												<span aria-hidden="true">&laquo;</span>
											</a>
										</c:when>
										<c:otherwise>
											<a href="adoptSearch.do?cpage=${pi.startPage - pi.boardLimit}&boardType=${boardType}&keyword=${keyword}" class="page-link"> 
												<span aria-hidden="true">&laquo;</span>
											</a>
										</c:otherwise>
									</c:choose></li>
						...
						<c:choose>
							<c:when test="${pi.currentPage > 1}">
								<li class="page-item"><c:choose>
										<c:when test="${category ne null}">
											<a href="adoptSearch.do?cpage=${pi.currentPage - 1}&boardType=${boardType}&category=${category}&keyword=${keyword}" class="page-link"> 
												<span aria-hidden="true">&lt;</span>
											</a>
										</c:when>
										<c:otherwise>
											<a href="adoptSearch.do?cpage=${pi.currentPage - 1}&boardType=${boardType}&keyword=${keyword}" class="page-link"> 
												<span aria-hidden="true">&lt;</span>
											</a>
										</c:otherwise>
									</c:choose></li>
							...
							<c:otherwise>
								<li class="page-item disabled"><c:choose>
										<c:when test="${category ne null}">
											<a
												href="adoptSearch.do?cpage=${pi.currentPage - 1}&boardType=${boardType}&category=${category}&keyword=${keyword}"
												class="page-link"> <span aria-hidden="true">&lt;</span>
											</a>
										</c:when>
										<c:otherwise>
											<a
												href="adoptSearch.do?cpage=${pi.currentPage - 1}&boardType=${boardType}&keyword=${keyword}"
												class="page-link"> <span aria-hidden="true">&lt;</span>
											</a>
										</c:otherwise>
									</c:choose></li>
							...
							<c:choose>
								<c:when test="${page == pi.currentPage}">
									<li class="page-item active"><c:choose>
											<c:when test="${category ne null}">
												<a class="page-link"
													href="adoptSearch.do?cpage=${page}&boardType=${boardType}&category=${category}&keyword=${keyword}">${page}</a>
											</c:when>
											<c:otherwise>
												<a class="page-link"
													href="adoptSearch.do?cpage=${page}&boardType=${boardType}&keyword=${keyword}">${page}</a>
											</c:otherwise>
										</c:choose></li>
								</c:when>
								<c:otherwise>
